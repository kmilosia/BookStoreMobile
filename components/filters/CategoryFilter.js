import { Column, Text } from "native-base";
import { COLORS } from "../../styles/constants";
import { Pressable } from "react-native";
import Ionicons from 'react-native-vector-icons/Ionicons'
import BouncyCheckbox from "react-native-bouncy-checkbox";
import { useEffect, useState } from "react";
import { getCategories } from "../../api/DictionaryAPI";

export default function CategoryFilter({filtersOpen,setFiltersOpen, filterElements, setFilterElements}){
    const [data, setData] = useState([])
    const handleCheckboxPress = (categoryId) => {
        const isSelected = filterElements.category.includes(`&CategoryIds=${categoryId}`);
        if (isSelected) {
          setFilterElements((prevValues) => ({
            ...prevValues,
            category: prevValues.category.replace(`&CategoryIds=${categoryId}`, ''),
        }));
        } else {
          setFilterElements((prevValues) => ({
            ...prevValues,
            category: `${prevValues.category}&CategoryIds=${categoryId}`,
        }));
        }
      };
    useEffect(() =>{
        getCategories(setData)
    },[])
    return(
        <Column marginBottom={4} borderWidth={2} borderColor={COLORS.triary} bg={COLORS.secondary} borderRadius={8} width='100%'>
            <Pressable onPress={() => setFiltersOpen({ ...filtersOpen, category: !filtersOpen.category })} style={{width: '100%',borderBottomWidth: filtersOpen.category ? 2 : 0,borderColor: COLORS.triary, paddingHorizontal: 14, paddingVertical: 10, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
                <Text fontWeight={500} fontSize={18} color='white'>Kategorie</Text>
                <Ionicons name='chevron-down' color='white' size={18} />
            </Pressable>
            {filtersOpen.category &&
            <Column width='100%' paddingX={4} paddingY={3}>
                {data?.map((item,index) => {
                    return(
                        <BouncyCheckbox
                        isChecked={filterElements.category.includes(`&CategoryIds=${item.id}`)}
                        onPress={() => {handleCheckboxPress(item.id)}}
                        key={index}
                        size={20}
                        fillColor={COLORS.accent}
                        unfillColor="#ffffff"
                        text={item.name}
                        marginBottom={8}
                        textStyle={{
                            color: 'white',
                            textDecorationLine: 'none'
                        }}
                        iconStyle={{
                            borderRadius: 4
                        }}
                        innerIconStyle={{
                            borderRadius: 4
                        }}
                        />
                    )
                })}
            </Column>
            }
        </Column>
    )
}